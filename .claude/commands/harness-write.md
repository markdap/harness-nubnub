...
- **사용자** / 페르소나
- **핵심 기능** — ***phase 체크리스트 형식*으로 1차 안 자동 박기** (3개 이하 권장):
  ...
  **필수**: 각 줄은 정확히 `- [ ] phase {N-name}: {설명}` 패턴. *공백·하이픈·콜론 위치 정확히* — 이 패턴이 `/harness-go §2-1` 가드의 매칭 기준이 됨.

  채운 후 사용자에게 컨펌:
  > *"phase 1차 안입니다. 단위·순서·이름 OK하시면 그대로 `/harness-go`로 진행. 수정 원하면 알려주세요."*

  사용자가 단위·순서·이름 수정 요청하면 그 자리에서 PRD §핵심 기능 갱신.
