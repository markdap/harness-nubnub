...
`handoffs/` 폴더에 `.md` 파일이 있으면 **가장 최근 파일 1개를 반드시 먼저 읽어라.**
이건 전역 `/handoff` 명령으로 이전 세션이 남긴 컨텍스트다.
이전 세션이 남긴 컨텍스트를 읽고, 이전 세션에서 하지 못한 부분과 현재 세션에서 해야 할 부분을 파악하라.
...
**docs 채우는 순서 (반드시 지킬 것):**

1. `docs/PRD.md` (목표 / 사용자 / 핵심 기능 / MVP 포함·제외 / v2 로드맵)
2. `docs/ARCHITECTURE.md` (기술 스택 / 폴더 구조 / 데이터 흐름)
3. `docs/ADR.md` (결정 근거 · 트레이드오프)
4. `CLAUDE.md` (위 세 파일에서 결정된 것 중 "AI가 코드 짜기 직전에 안 읽으면 사고나는 규칙"만 요약)

**CLAUDE.md는 반드시 마지막.** PRD/ARCHITECTURE/ADR이 일정 수준 채워지기 전에는 CLAUDE.md를 건드리지 마라. 이유: 다른 파일이 비어있으면 "어차피 다른 데 안 적었으니 일단 여기"라는 충동으로 CLAUDE.md가 비대해진다.

CLAUDE.md를 채울 차례가 되면 상단 주석 블록의 분리 규칙을 따르고, 채우기가 끝나면 그 가이드 주석 블록과 예시 블록을 모두 **제거**하라. 헌법 안에 메타가 영구 잔존하면 안 된다.
