...
Harness 워크플로우 (5단계 사이클):
  1. /grill-with-docs       ← 아이디어 깊이 인터뷰 (진척률 N/M 자동)
  2. /design-consultation   ← DESIGN.md 합성 (raw는 docs/design-references/, 이번 스킵 명시 가능)
  3. /harness-write         ← 그릴+디자인 결과 → docs 자동 분배 매핑 (DESIGN.md를 input으로)
  4. /harness-go            ← phase 설계 + execute.py 자동 실행 + GitHub Issue 발행
...
- `/grill-with-docs` 행동 보강: 첫 라운드에 *이 프로젝트의 큰 결정 트리 N개*를 사용자와 함께 정의하고, 매 라운드 말미에 *진척률 N/M*을 표시. M=N일 때 *그릴 종료 + `/design-consultation` 권유* 안내를 자동 출력.
