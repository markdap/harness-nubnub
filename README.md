...
GitHub 레포는 *나중에* 만든다. 살아남을지 모르는 실험에 GitHub 노이즈 안 쌓고, *살아남은 후* 한 줄로 연결.

```bash
# 1. 템플릿을 새 폴더로 복사 (degit이 .git 빼고 깔끔하게 가져옴)
npx degit markdap/harness-nubnub my-project
cd my-project

# 2. 로컬 git 시작 (GitHub 아직 X)
git init

# 3. cmux/터미널 켜고 claude 시작
claude
```

→ 그 다음 `/grill-with-docs`로 시작.

**왜 GitHub 먼저 안 만드나?**
- 실패하는 실험에 GitHub 시체 안 쌓임
- 폴더 단순 삭제로 가볍게 끝낼 수 있음
- 살아남은 후 한 줄로 연결 → 모든 GitHub 가치(Issue 자동 발행, 백업) 즉시 회수

### 살아남으면 GitHub에 올리기 (한 줄)

```bash
gh repo create my-project --private --source=. --push
```

→ 이 시점부터 `/harness-go`가 phase 만들 때마다 **GitHub Issue 자동 발행**도 켜짐. 그 전까진 발행 단계만 *건너뜀* (안내 메시지 출력 후 phase 정상 진행).

### 처음부터 GitHub로 시작 (외부 공개/팀 합류 명확할 때만)

1. https://github.com/markdap/harness-nubnub 접속
2. **"Use this template"** → **"Create a new repository"** → Private 선택
3. clone → cd → `claude`

→ 처음부터 Issue 자동 발행 켜진 상태로 사이클 시작.
